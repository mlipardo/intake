...
describe('ShowField', () => {
  ...
  describe('when only required props are passed', () => {
    it('renders a label with a div wrapper', () => {
      ...
      expect(component.html())
        .toEqual('<div class=""><div class="show-label">L1</div><div></div><div></div><span class="text-area-show"><div>Italy</div></span></div>')
    })
  })
  ...
  describe('when label and className props are passed', () => {
    ...
    it('renders the label inside the grid wrapper with the classes', () => {
      component = shallow(<ShowField {...props}/>, {disableLifecycleMethods: true})
      expect(component.html())
        .toEqual('<div class="giggidy"><div class="working-class object-oriented-class show-label">Do not judge a component by its label</div><div></div><div></div><span class="text-area-show"><br/></span></div>')
  ...
  describe('when children are passed', () => {
    ...
    it('renders the children between the label and ErrorMessages', () => {
      ...
      expect(wrapper.childAt(3).html()).toEqual('<span class="text-area-show"><h1>Child</h1></span>')
